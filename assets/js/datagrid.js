...
/* JSHint warns that DataGrid, dataGridAdjustForScrollBars, dataGridDisplaySetOnClick, dataGridDisplaySetCols, dataGridDisplayClicked, dataGridDisplayRefresh */
/* are unused.  These are called externally from this file. */
...
function dataGridHeaderContentColCountMatch(hLen, cLen) {
    if (hLen !== cLen) {
        ...
        if (hLen < cLen) { //Data columns outnumber headers, so return false
...
function DataGrid(headers, content) {
    ...
    for (i = 0; i < rowCount; i += 1) {
        if (eachRowCount[i] < rowCount) {
            for (ii = 0; ii < (rowCount - eachRowCount[i]); ii += 1) {
...
function dataGridDisplayGetCounts(dataGridDisplayId) {
    ...
    let i;
    for (i = 0; i < document.getElementById(dataGridDisplayId).classList.length; i += 1) {
...
function dataGridDisplaySetCols(dataGridDisplayId, cols) {
    ...
    if (Number.isNaN(cols) === false && cols !== "") {
        ...
        for (ii = 0; ii < elementCollection[i].classList.length; ii += 1) { //Iterate through the classList of each element
            if (elementCollection[i].classList.item(ii).slice(0, 3) === "col") {
                if (elementCollection[i].classList.item(ii).length === 3) {
                    ...
                    if (Number.isNaN(bootstrapColClass) === true) {
                ...
                if (Number.isNaN(datagridCustomClass) === true) {
...
function dataGridDisplaySetSelected(dataGridDisplayId, count, selId, option) {
    ...
    if (option !== "row" && option !== "col") { //If option is not row or col, then it is invalid so return false
        console.log(`function dataGridDisplaySetSelected failed.  Provided option value (${option}) is invalid.  Value can either be 'row' or 'col'.`);
        return false;
    }
